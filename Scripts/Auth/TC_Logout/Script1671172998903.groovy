...
not_run: response = WS.sendRequest(findTestObject('Login/1positif/Login valid account'), FailureHandling.CONTINUE_ON_FAILURE)

not_run: JsonSlurper slurper = new JsonSlurper()

not_run: Map parsedJson = slurper.parseText(response.getResponseText())

not_run: String Token = parsedJson.data.access_token

not_run: GlobalVariable.UserToken = Token

not_run: println(GlobalVariable.UserToken)
...
response = WS.sendRequest(findTestObject('Logout/Logout admin'), FailureHandling.CONTINUE_ON_FAILURE)

WS.verifyResponseStatusCode(response, 200, FailureHandling.CONTINUE_ON_FAILURE)

response = WS.sendRequest(findTestObject('Logout/Logout superadmin'), FailureHandling.CONTINUE_ON_FAILURE)
